...
CumulativeARI <- function(embedding, knownlabels, c.param = NA){
  ARIs <- c()
   for (i in seq_along(ncol(embedding))) {
     walktrapClusters <- get_walktrapClusters(embedding[,seq_len(i)], c.param)
...
CumulativeFstatistic <- function(embedding, labels) {
  ...
  for (i in seq_along(TSS)){
    Fstat <- c(Fstat, (sum(TSS[seq_len(i)]) - sum(RSS[seq_len(i)]))/sum(RSS[seq_len(i)]))
  }
  return(Fstat)
}
```
...
for (i in seq_along(nrow(sce_SC4_qc@colData))) {
...
for (i in seq_along(nrow(sce_SC1_qc@colData))) {
...
for (i in seq_along(nrow(sce_SC2_qc@colData))) {
...
for (i in seq_along(nrow(sce_SC3_qc@colData))) {
...
sc1PCA <- ggplot(data=as.data.frame(reducedDim(sce_SC1_qc,"PCA")),aes(x=PC1,y=PC2,col=sce_SC1_qc$group))+
  geom_point(size=1.2, show.legend = FALSE, alpha=0.7)+
