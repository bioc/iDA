...
# Pheno Data
```{r}
destfile <- "E-GEUV-3.sdrf.txt"
download.file(url = "https://www.ebi.ac.uk/arrayexpress/files/E-GEUV-3/E-GEUV-3.sdrf.txt", destfile = destfile)

#pheno data
unduped_samples = read.csv("~/Desktop/iDA_bioc/data/sampleids.csv")
coldata <- read.delim("E-GEUV-3.sdrf.txt", header = TRUE)
coldata <- coldata %>%
  select(Factor.Value.population., Factor.Value.laboratory., Assay.Name) %>%
  unique() 

rownames(coldata) <- coldata$Assay.Name
head(coldata)
colnames(coldata) <- c("Population", "Laboratory", "AssayName")
...
https://www.ebi.ac.uk/arrayexpress/experiments/E-GEUV-3/files/analysis_results/?ref=E-GEUV-3
```{r}
#read in data table
GVD_raw <- read.table("~/Downloads/GD660.GeneQuantCount.txt", header = TRUE, row.names = "Gene_Symbol")
#subset columns for sample counts and remove duplicate samples
GVD_raw <- GVD_raw[, colnames(GVD_raw) %in% coldata$Assay.Name]
...
coldata <- coldata[colnames(GVD_raw),]
